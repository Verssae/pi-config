...
- `bootstrap.sh` — 심링크 생성 및 `packages.json` 항목 설치

`~/.pi/agent`에는 런타임 상태(`sessions`, `missions`, `npm`, `git`, `auth.json` 등)만 로컬로 둔다.
...
`package.json`이 있는 확장은 의존성을 수동 설치한다(아래 의존성 참고).

### 일반 사용자용 (부분 복사)

한 덩어리로 설치하는 용도가 아니다. 원하는 조각만 골라 자신의 pi 설정으로 복사하라.

일부 확장은 별도 저장소에서 왔다:

- **[pi-interactive-subagents](https://github.com/amosblomqvist/pi-interactive-subagents)** — 이 저장소에 vendoring(내장)했고, tmux 대신 Herdr pane 백엔드로 적응시켰다
- **[pi-web-access](https://github.com/nicobailon/pi-web-access)** — 웹 검색/추출/검증 패키지 (npm으로 설치)
- **[pi-observational-memory](https://github.com/Verssae/pi-observational-memory)** — 계층형 세션 기억 + 결정론적 컴팩션 (패치된 fork)
- **[pi-lsp-adapter](https://github.com/Verssae/pi-lsp-adapter)** — LSP 도구 제공 (패치된 fork)
...
`pi-observational-memory`는 이 디렉터리에 없다 — [Verssae/pi-observational-memory](https://github.com/Verssae/pi-observational-memory) fork가 `packages.json`의 `git:` 항목으로 설치된다. 웹 접근은 `npm:pi-web-access`가 담당하며, 폐기된 로컬 `web-fetch/`, `web-search/`는 `deprecated/extensions/`에 보관된다.
...
- `youtube-transcript/` — 유튜브 제목과 자막을 JSON으로 가져오기
...
`deprecated/`는 두 달짜리 셋업 이후 활용하지 않게 된 확장/스킬 보관소다. 동작은 하지만 자리를 잃은 것들이며, 참고용으로 유지한다.
...
`youtube-transcript/`가 사용한다. `youtube-transcript/`와 `analyze-sessions/`에는 Python 3이 필요하다(표준 라이브러리만 사용).
