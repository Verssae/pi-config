 ...
 *
 * Run from a shell pane inside Herdr:
...
for (const backend of backends) {
  describe(`Herdr-surface [${backend}]`, { timeout: 60_000 }, () => {
    ...
    it("keeps focus unchanged while creating and targeting subagent surfaces", async () => {
      const initialFocus = getFocusedSurface();
      assert.ok(initialFocus, "Expected a focused Herdr pane");
