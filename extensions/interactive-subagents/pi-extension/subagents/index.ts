...
function addHerdrStateExtension(parts: string[]): void {
  if (process.env.HERDR_ENV !== "1") return;
  const extensionPath = join(getAgentConfigDir(), "extensions", "herdr-agent-state.ts");
  if (existsSync(extensionPath)) parts.push("-e", shellEscape(extensionPath));
}
