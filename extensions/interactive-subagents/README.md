...
Async subagents for [pi](https://github.com/earendil-works/pi), running in [Herdr](https://herdr.dev) panes. Spawn a sub-agent, keep working in the main session, and get the result steered back when it finishes. Fully non-blocking.

**Herdr adaptation.** This vendored fork replaces the upstream tmux surface with Herdr's stable pane CLI and preserves focus with `--no-focus`.

## How it works

`subagent()` returns immediately. The sub-agent runs in its own Herdr pane — split right or down from the parent Pi pane according to its current geometry, without stealing keyboard focus. A live widget above the input tracks every running sub-agent, and when one finishes, its result is steered into the main session as a notification that triggers a new turn.
...
Herdr owns layout. The extension chooses a right split for wide panes and a downward split for narrow panes; explicit left/up requests are implemented by splitting and swapping panes.
...
| **researcher** | `openai-codex/gpt-5.6-terra` | `web_search`, `fetch_content`, `get_search_content` | Web research, synthesized into a sourced brief |
| **worker** | `openai-codex/gpt-5.6-sol` | `read`, `write`, `edit`, `bash`, web-access tools + spawning | General implementer; may spawn `scout` and `researcher` |
...
| `tools` | string | Strict tool allowlist. Built-ins: `read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`. Extension-backed: `web_search`, `fetch_content`, `get_search_content`, `source_check`, `safe_bash`, and registered custom tools. Only the extensions backing the listed tools are loaded into the child |
...
- [Herdr](https://herdr.dev), with Pi launched inside a Herdr-managed pane (`HERDR_ENV=1`)

The extension uses `herdr pane split/run/read/close` and explicitly loads Herdr's managed `herdr-agent-state.ts` integration into restricted child Pi sessions.

## Acknowledgements

Adapted from [amosblomqvist/pi-interactive-subagents](https://github.com/amosblomqvist/pi-interactive-subagents), itself forked from [HazAT/pi-interactive-subagents](https://github.com/HazAT/pi-interactive-subagents). The original projects established the subagent architecture and status widget; this fork replaces the terminal surface with Herdr.
